...
//  for (i=0; programList[i]!=NULL && i<PROGRAMSMAXNUMBER ; i++) {
		...
		} else if (PID == BIGGERTHAN3){
			OperatingSystem_ShowTime(EXAM);
			ComputerSystem_DebugMessage(108, EXAM, "Más de 3 tics de reloj desde su llegada");
			ComputerSystem_DebugMessage(108, EXAM, "\n");
		}
		 else{
...
int OperatingSystem_CreateProcess(int indexOfExecutableProgram) {
	...
	if(((executableProgram->arrivalTime) - Clock_GetTime())> 3){
		return BIGGERTHAN3;
	}
