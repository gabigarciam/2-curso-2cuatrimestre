...
#include "Messages.h"
#include "Asserts.h"
#include "Clock.h" // V2-studentsCode
...
int ComputerSystem_PrepareAditionalPrograms(int programListNextPosition, int type, char * programsFileName){
	...
	ComputerSystem_DebugMessage(77,POWERON,numberOfPrograms,type==DAEMONPROGRAM ? "daemon":"user", programsFileName);

	return programListNextPosition;
}

// Similar to OperatingSystem_ShowTime in ComputerSystem
// Don't tab the line
void ComputerSystem_ShowTime(char section) { // V2-studentsCode
...
void ComputerSystem_PrintArrivalTimeQueue(){ // V3-studentsCode
  ...
  if (numberOfProgramsInArrivalTimeQueue>0) {
	for (i=0; i< numberOfProgramsInArrivalTimeQueue; i++) {
	  ...
	  ComputerSystem_DebugMessage(70,LONGTERMSCHEDULE,programList[arrivalTimeQueue[i].info]->executableName,
