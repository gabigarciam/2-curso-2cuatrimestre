...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		case WRITE_INST: 
			...
			// Send to the main memory controller the data to be written: 
			// use the data bus for this
			// envía el dato del mbr de la cpu a la main memory
		...
		case READ_INST: 
			...
			registerMAR_CPU=operand1;
			// Send to the main memory controller the address in which 
			// the reading has to take place: use the address bus for this 
			// (envía el dato del registro MAR)
			Buses_write_AddressBus_From_To(CPU, MAINMEMORY);
			// Tell the main memory controller to read
			registerCTRL_CPU=CTRLREAD;
			// Send to the main memory controller the operation
