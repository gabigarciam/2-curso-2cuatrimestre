...
//  for (i=0; programList[i]!=NULL && i<PROGRAMSMAXNUMBER ; i++) {
		...
		if(PID == NOFREEENTRY){ 
			...
			//TODO: Aquí, esta asignación no está bien
			programList[i]->arrivalTime += 25;
			//Añadir a la cola el programa de arrival el programa
			Heap_add(programList[i]->arrivalTime, arrivalTimeQueue, QUEUE_ARRIVAL, 
			&(numberOfProgramsInArrivalTimeQueue), PROGRAMSMAXNUMBER);
...
int OperatingSystem_ObtainMainMemory(int processSize, int PID) {
	...
	int bestPartitionSize = 10000000;
	int bestPartition = -1;
	for(int i = 0; i < numberOfPartitions; i++){
	// for(int i = 0; i < PARTITIONTABLEMAXSIZE; i++){
		if(partitionsTable[i].size >= processSize && partitionsTable[i].size < bestPartitionSize
		&& partitionsTable[i].PID == NOPROCESS){
			bestPartitionSize = partitionsTable[i].size;
			bestPartition = i;
		}
	}
	if(bestPartition == -1){
